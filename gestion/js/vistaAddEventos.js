...
/* Busqueda de administradores por suggeest list */
 function lookup(inputString,suggestionbox,suggestionboxlist,url) { 
	...
	function busqueda_evento(valor){
	   var jqxhr = $.post(ruta+"wp-admin/admin-ajax.php","action=caja_busca_evento&accion=buscaEventos&busqueda="+valor,   	   
	   function(data){ 
		          $("#frmBusqueda").html(data);	
	   });   
	}
	function busqueda_evento_orden(valor,orden){
